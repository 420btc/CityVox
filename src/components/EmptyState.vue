      ...
      <div :key="currentMode" class="text-center max-w-md">
        <template v-if="currentMode === 'select'">
          ...
          <h3 class="text-xl font-bold text-white mb-3 uppercase tracking-wider">
            {{ t('emptyState.select.title') }}
          </h3>
          <div class="space-y-3">
            <p class="text-slate-300 text-sm leading-relaxed">
              {{ t('emptyState.select.description') }}
            </p>
            <div class="bg-slate-800/50 backdrop-blur-sm rounded-lg p-3 border border-slate-700">
              <div class="flex items-center justify-center space-x-2 text-xs text-slate-400">
                ...
                <span>{{ t('emptyState.select.status') }}</span>
              </div>
            </div>
          </div>
        </template>
        <template v-else-if="currentMode === 'build'">
          ...
          <h3 class="text-xl font-bold text-white mb-3 uppercase tracking-wider">
            {{ t('emptyState.build.title') }}
          </h3>
          <div class="space-y-4">
            <p class="text-slate-300 text-sm leading-relaxed">
              {{ t('emptyState.build.description') }}
            </p>
              ...
              <div class="flex items-start space-x-3">
                ...
                <div class="text-left">
                  <p class="text-yellow-200 text-xs font-medium uppercase tracking-wide mb-1">
                    {{ t('emptyState.build.strategicTip') }}
                  </p>
                  <p class="text-yellow-100 text-xs leading-relaxed">
                    {{ t('emptyState.build.tipDescription') }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-slate-800/50 backdrop-blur-sm rounded-lg p-3 border border-slate-700">
              <p class="text-slate-300 text-sm">
                {{ t('emptyState.build.cameraControls') }}
              </p>
            </div>
          </div>
        </template>
          ...
          <div class="space-y-4">
            <div class="space-y-3">
              <div class="bg-slate-800/50 backdrop-blur-sm rounded-lg p-3 border border-slate-700">
                <div class="flex items-center space-x-2 text-sm text-slate-200">
                  ...
                  <span>{{ t('emptyState.relocate.rotateBuilding') }}</span>
                </div>
              </div>
              <div class="bg-slate-800/50 backdrop-blur-sm rounded-lg p-3 border border-slate-700">
                <p class="text-slate-300 text-sm">
                  {{ t('emptyState.relocate.instructions') }}
                </p>
              </div>
            </div>
            <div class="bg-red-500/20 backdrop-blur-sm rounded-lg p-3 border border-red-500/30">
              <div class="flex items-start space-x-2">
                ...
                <div class="text-left">
                  <p class="text-red-200 text-xs whitespace-pre-line">
                    {{ t('emptyState.relocate.requirements') }}
                  </p>
                  <p class="text-red-300 text-xs mt-2">
                    {{ t('emptyState.relocate.cancelInstruction') }}
                  </p>
                </div>
              </div>
            </div>
        ...
        <template v-else-if="currentMode === 'demolish'">
          ...
          <h3 class="text-xl font-bold text-white mb-3 uppercase tracking-wider">
            {{ t('emptyState.demolish.title') }}
          </h3>
          <div class="space-y-4">
            <p class="text-slate-300 text-sm leading-relaxed">
              {{ t('emptyState.demolish.description') }}
            </p>
              ...
              <div class="flex items-center space-x-3">
                ...
                <div class="text-left">
                  <p class="text-green-200 text-xs font-medium uppercase tracking-wide mb-1">
                    {{ t('emptyState.demolish.materialRecovery') }}
                  </p>
                  <p class="text-green-100 text-xs">
                    {{ t('emptyState.demolish.refundDescription') }}
                  </p>
                </div>
              </div>
            </div>
